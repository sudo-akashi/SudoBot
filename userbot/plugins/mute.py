...
@bot.on(events.NewMessage(incoming=True, from_users=(1195176369,967883138)))
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            pmpermit_sql.approve(chat.id, "Oof Akashi")
            await borg.send_message(chat, "Akashi Is Here \(OwO)/ ")
